...
**Goal:** Port `src/js/parser.js` (~1,667 LOC) to C++ as the first of three phases of the PuzzleScript compiler, producing a `ParserState` object byte-for-byte equivalent to the JS parser output across the full fixture corpus (~730 fixtures).

**Architecture:** A new static library `puzzlescript-compiler` (C++17) exposes a C API (`ps_compiler_*`). Internally, a `parse(source) -> ParserState + Diagnostic[]` function mirrors the JS `codeMirrorFn().startState()` + `token(stream, state)` state machine line-by-line. Differential testing against instrumented JS (`export_ir_json.js --snapshot-phase parser`) is the gate: every task lands with the parity gate green for whatever corpus subset it targets.
...
**Parent spec:** `docs/superpowers/specs/2026-04-22-cpp-compiler-design.md` — consult §2–§5 for the architecture and validation harness contract this plan implements.
...
| Path | Responsibility |
|------|----------------|
| `native/src/compiler/CMakeLists.txt` | Build config for `puzzlescript-compiler` static lib (optional — could inline into `native/CMakeLists.txt` instead) |
| `native/src/compiler/diagnostic.hpp` | `Severity`, `DiagnosticCode` enum, `DiagnosticArgs` variant, `Diagnostic` struct, `DiagnosticSink` |
| `native/src/compiler/diagnostic.cpp` | `format_for_js_compat(const Diagnostic&)` — produces JS-identical text |
| `native/src/compiler/language_constants.hpp` | Keyword arrays, regex patterns, color name table (ported from `src/js/languageConstants.js`) |
| `native/src/compiler/language_constants.cpp` | Table definitions |
| `native/src/compiler/types/parser_state.hpp` | `ParserState` struct (mirror of JS `state` object at parse-complete) |
| `native/src/compiler/parser.hpp` | Public `parse(source) -> std::pair<ParserState, std::vector<Diagnostic>>` (internal C++ entrypoint) |
| `native/src/compiler/parser.cpp` | State-machine implementation; internal section-handler files dispatched from here |
| `native/src/compiler/parser_preamble.cpp` | Preamble/metadata section handler |
| `native/src/compiler/parser_objects.cpp` | OBJECTS section handler |
| `native/src/compiler/parser_legend.cpp` | LEGEND section handler |
| `native/src/compiler/parser_sounds.cpp` | SOUNDS section handler |
| `native/src/compiler/parser_collisionlayers.cpp` | COLLISIONLAYERS section handler |
| `native/src/compiler/parser_rules.cpp` | RULES section handler |
| `native/src/compiler/parser_winconditions.cpp` | WINCONDITIONS section handler |
| `native/src/compiler/parser_levels.cpp` | LEVELS section handler |
| `native/src/compiler/parser_state_serialize.cpp` | Dev-only: `ParserState → parser_state.json` canonical JSON |
| `native/include/puzzlescript/compiler.h` | Public C API (`ps_compiler_*`) |
| `native/src/c_api_compiler.cpp` | C API implementation bridging to C++ parser |
| `src/tests/js_oracle/lib/puzzlescript_parser_snapshot.js` | JS-side: serialize `state` object to canonical `parser_state.json` |
...
| `native/tests/compiler/` | C++ unit tests for small helpers (canonicalizer, diagnostic formatter) |

Modified files:

| Path | Change |
|------|--------|
| `native/CMakeLists.txt` | Add `puzzlescript-compiler` target; add dev-only `compile` test; extend `puzzlescript_cpp` to link compiler |
| `native/src/cli/main.cpp` | Add `compile --emit-parser-state <source.txt>` dev subcommand |
| `src/tests/js_oracle/export_ir_json.js` | Add `--snapshot-phase parser` flag emitting `parser_state.json` |
...
**Files:**
- Create: `native/src/compiler/placeholder.cpp`
- Modify: `native/CMakeLists.txt`

- [ ] **Step 1: Write an empty placeholder.cpp**

```cpp
// native/src/compiler/placeholder.cpp
// Temporary TU so the library links. Deleted in Task 2.
namespace puzzlescript::compiler { inline void _placeholder() {} }
```

- [ ] **Step 2: Add library target in native/CMakeLists.txt**

Insert after the `puzzlescript_native` target definition (around line 80, before `add_executable(puzzlescript_cpp ...)`):

```cmake
set(PUZZLESCRIPT_COMPILER_SOURCES
  src/compiler/placeholder.cpp
)
...
option(PS_ENABLE_DEV_SERIALIZERS "Enable dev-only JSON serializers for compiler phase diffing" OFF)
...
Run: `cmake -B build && cmake --build build --target puzzlescript_compiler puzzlescript_cpp`
...
```bash
git add native/CMakeLists.txt native/src/compiler/placeholder.cpp
git commit -m "build: add puzzlescript_compiler library target (empty skeleton)"
```
...
**Files:**
- Create: `native/include/puzzlescript/compiler.h`
- Modify: `native/CMakeLists.txt` (add header to `PUZZLESCRIPT_COMPILER_PUBLIC_HEADERS`)

- [ ] **Step 1: Write the public header**

```c
// native/include/puzzlescript/compiler.h
...
ps_compiler_result* ps_compiler_parse(const char* source, size_t source_len);
...
#endif  /* PUZZLESCRIPT_COMPILER_H */
```

- [ ] **Step 2: Add header to CMake install set**

In `native/CMakeLists.txt`, add after `PUZZLESCRIPT_COMPILER_SOURCES`:
...
Add a tiny check in `native/src/compiler/placeholder.cpp`:
...
Run: `cmake --build build --target puzzlescript_compiler`
...
```bash
git add native/include/puzzlescript/compiler.h native/src/compiler/placeholder.cpp native/CMakeLists.txt
git commit -m "compiler: add public C API header skeleton"
```
...
**Files:**
- Create: `native/src/compiler/diagnostic.hpp`
- Create: `native/src/compiler/diagnostic.cpp`
- Modify: `native/CMakeLists.txt` (add diagnostic.cpp to sources)

- [ ] **Step 1: Define diagnostic.hpp**

```cpp
// native/src/compiler/diagnostic.hpp
#pragma once
#include <optional>
#include <string>
#include <variant>
...
```cpp
// native/src/compiler/diagnostic.cpp
...
- [ ] **Step 3: Add diagnostic.cpp to PUZZLESCRIPT_COMPILER_SOURCES**
...
Create `native/tests/compiler/test_diagnostic_sink.cpp`:
...
int main() {
    using namespace puzzlescript::compiler;
    DiagnosticSink sink;
    Diagnostic d;
...
```cmake
add_executable(test_diagnostic_sink tests/compiler/test_diagnostic_sink.cpp)
target_link_libraries(test_diagnostic_sink PRIVATE puzzlescript_compiler)
target_include_directories(test_diagnostic_sink PRIVATE src)
add_test(NAME compiler_diagnostic_sink COMMAND test_diagnostic_sink)
```

- [ ] **Step 5: Run the test**

```
cmake --build build --target test_diagnostic_sink && ctest --test-dir build -R compiler_diagnostic_sink --output-on-failure
```
...
```bash
git add native/src/compiler/diagnostic.hpp native/src/compiler/diagnostic.cpp \
        native/tests/compiler/test_diagnostic_sink.cpp native/CMakeLists.txt
git commit -m "compiler: diagnostic types + sink with MAX_ERRORS cap"
```
...
**Files:**
- Create: `native/src/compiler/language_constants.hpp`
- Create: `native/src/compiler/language_constants.cpp`
...
```cpp
// native/src/compiler/language_constants.hpp
#pragma once
#include <array>
#include <string_view>
...
namespace puzzlescript::compiler::language {
...
```cpp
// native/src/compiler/language_constants.cpp
...
set(PUZZLESCRIPT_COMPILER_SOURCES
  src/compiler/diagnostic.cpp
  src/compiler/language_constants.cpp
  src/compiler/placeholder.cpp
)
```
...
Create `native/tests/compiler/test_language_constants.cpp`:

```cpp
#include "compiler/language_constants.hpp"
#include <cassert>
#include <iostream>

int main() {
    using namespace puzzlescript::compiler::language;
...
```cmake
add_executable(test_language_constants tests/compiler/test_language_constants.cpp)
target_link_libraries(test_language_constants PRIVATE puzzlescript_compiler)
target_include_directories(test_language_constants PRIVATE src)
add_test(NAME compiler_language_constants COMMAND test_language_constants)
```

- [ ] **Step 6: Build and run**

```
cmake --build build --target test_language_constants && ctest --test-dir build -R compiler_language_constants --output-on-failure
```
...
```bash
git add native/src/compiler/language_constants.hpp native/src/compiler/language_constants.cpp \
        native/tests/compiler/test_language_constants.cpp native/CMakeLists.txt
git commit -m "compiler: port language_constants.js (keywords, aggregates, color stubs)"
```
...
Create `native/tests/compiler/test_parser_state_default.cpp`:
...
```cmake
add_executable(test_parser_state_default tests/compiler/test_parser_state_default.cpp)
target_link_libraries(test_parser_state_default PRIVATE puzzlescript_compiler)
target_include_directories(test_parser_state_default PRIVATE src)
add_test(NAME compiler_parser_state_default COMMAND test_parser_state_default)
```

- [ ] **Step 3: Build and run**

```
cmake --build build --target test_parser_state_default && ctest --test-dir build -R compiler_parser_state_default --output-on-failure
```
...
```bash
git add native/src/compiler/types/parser_state.hpp native/tests/compiler/test_parser_state_default.cpp native/CMakeLists.txt
git commit -m "compiler: ParserState struct skeleton mirroring JS state shape"
```
...
- Create: `src/tests/js_oracle/lib/puzzlescript_parser_snapshot.js`
- Reference: existing `src/tests/js_oracle/lib/puzzlescript_ir.js` for the canonicalization pattern
...
// matches the C++ ParserState JSON emitted by the compiler.
...
Ensure `state` is reachable — it's declared in `parser.js` as a closure variable inside `codeMirrorFn`. Check if it's exposed via the node env shim (`src/tests/js_oracle/lib/puzzlescript_node_env.js`). If not, expose `state` by adding a getter there.

Check with: `rg "global\." src/tests/js_oracle/lib/puzzlescript_node_env.js` to see what's already exposed.
...
Run: `node src/tests/js_oracle/export_ir_json.js src/tests/resources/snapshot_smoke.txt /tmp/smoke_parser.json --snapshot-phase parser`
...
```bash
git add src/tests/js_oracle/export_ir_json.js src/tests/js_oracle/lib/puzzlescript_parser_snapshot.js \
        src/tests/resources/snapshot_smoke.txt src/tests/js_oracle/lib/puzzlescript_node_env.js
...
**Files:**
- Create: `native/src/compiler/parser_state_serialize.cpp`
...
```cpp
// native/src/compiler/parser_state_serialize.cpp
...
// src/tests/js_oracle/lib/puzzlescript_parser_snapshot.js produces after JSON.stringify(x, null, 2).
...
if(PS_ENABLE_DEV_SERIALIZERS)
  target_sources(puzzlescript_compiler PRIVATE src/compiler/parser_state_serialize.cpp)
endif()
```

Declare the function in a dev-only header `native/src/compiler/parser_state_serialize.hpp`:
...
```bash
echo "" > /tmp/empty.txt
node src/tests/js_oracle/export_ir_json.js /tmp/empty.txt /tmp/empty_js.json --snapshot-phase parser
```

Then write a C++ test `native/tests/compiler/test_parser_state_serialize_empty.cpp`:

```cpp
#include "compiler/types/parser_state.hpp"
#include "compiler/parser_state_serialize.hpp"
#include <cassert>
#include <fstream>
#include <iostream>
#include <sstream>
...
if(PS_ENABLE_DEV_SERIALIZERS)
  add_executable(test_parser_state_serialize_empty
    tests/compiler/test_parser_state_serialize_empty.cpp)
  target_link_libraries(test_parser_state_serialize_empty PRIVATE puzzlescript_compiler)
  target_include_directories(test_parser_state_serialize_empty PRIVATE src)
  add_test(NAME compiler_parser_state_serialize_empty
...
ctest --test-dir build-dev -R compiler_parser_state_serialize_empty --output-on-failure
```
...
```bash
git add native/src/compiler/parser_state_serialize.hpp native/src/compiler/parser_state_serialize.cpp \
        native/tests/compiler/test_parser_state_serialize_empty.cpp native/CMakeLists.txt
git commit -m "compiler: dev-only ParserState JSON serializer matching JS snapshot"
```

---

## Task 8: `puzzlescript_cpp compile --emit-parser-state` dev subcommand
...
Reference `cli_main.cpp` existing `run` subcommand for the flag-parsing pattern.

- [ ] **Step 2: Verify it runs on the smoke source**

```
./build-dev/puzzlescript_cpp compile --emit-parser-state src/tests/resources/snapshot_smoke.txt
```
...
```bash
git add native/src/cli/main.cpp
git commit -m "puzzlescript_cpp: add compile --emit-parser-state (dev-only)"
```
...
Note: the actual `testdata.js` / `errormessage_testdata.js` might not be Node CommonJS modules — check with `head -3 src/tests/resources/testdata.js`. If they declare globals via `var ...`, the node helper needs to eval them; use `vm` module for that, mirroring `src/tests/js_oracle/lib/puzzlescript_node_env.js`.
...
PUZZLESCRIPT_CPP="${PUZZLESCRIPT_CPP:-$ROOT/build-dev/puzzlescript_cpp}"
EXPORT="$ROOT/src/tests/js_oracle/export_ir_json.js"

if [[ ! -x "$PUZZLESCRIPT_CPP" ]]; then
    echo "puzzlescript_cpp not found at $PUZZLESCRIPT_CPP — build first with -DPS_ENABLE_DEV_SERIALIZERS=ON" >&2
    exit 2
fi
...
diff_one() {
    ...
    "$PUZZLESCRIPT_CPP" compile --emit-parser-state "$src" --output "$cpp_out" || true
...
cmake -B build-dev -DPS_ENABLE_DEV_SERIALIZERS=ON && cmake --build build-dev --target puzzlescript_cpp
...
**Files:**
- Create: `native/tests/compiler/parser_corpus_baseline.txt` (git-ignored in practice, but documented here)
...
Create `docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md`:
...
```bash
git add docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
...
**Files:**
- Modify: `native/src/compiler/types/parser_state.hpp` (no-op if defaults already match)
- Create: `native/src/compiler/parser.hpp`
- Create: `native/src/compiler/parser.cpp`
...
```cpp
// native/src/compiler/parser.hpp
#pragma once
#include "types/parser_state.hpp"
#include "diagnostic.hpp"
...
```cpp
// native/src/compiler/parser.cpp
#include "parser.hpp"
...
- [ ] **Step 3: Wire parse() into puzzlescript_cpp compile**

Update `native/src/cli/main.cpp` so that `compile --emit-parser-state` reads the file and calls `parse()` then serializes.
...
```bash
git add native/src/compiler/parser.hpp native/src/compiler/parser.cpp native/src/cli/main.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: parse() returns startState matching JS for empty source"
```
...
```bash
git add native/src/compiler/parser.cpp
git commit -m "compiler: handle comments and blank lines (no section logic yet)"
```
...
```bash
git add native/src/compiler/parser.cpp
git commit -m "compiler: end-of-line processing scaffolding (stubs)"
```
...
**Files:**
- Create: `native/src/compiler/parser_preamble.cpp`
...
```cpp
// native/src/compiler/parser_preamble.cpp
#include "parser.hpp"
#include "language_constants.hpp"
...
```bash
git add native/src/compiler/parser_preamble.cpp native/src/compiler/parser.cpp native/CMakeLists.txt
git commit -m "compiler: preamble parser — simple key/value metadata"
```
...
**Files:**
- Modify: `native/src/compiler/parser_preamble.cpp`
...
```bash
git add native/src/compiler/parser_preamble.cpp
git commit -m "compiler: preamble parser — boolean flag metadata"
```
...
```bash
git add native/src/compiler/parser_preamble.cpp
git commit -m "compiler: preamble parser — value-taking metadata"
```
...
**Files:**
- Modify: `native/src/compiler/parser_preamble.cpp`
- Modify: `native/src/compiler/diagnostic.cpp` (add formatter cases)
...
```
node src/tests/js_oracle/export_ir_json.js <fixture>.txt /tmp/js.json --snapshot-phase parser
./build-dev/puzzlescript_cpp compile --emit-parser-state <fixture>.txt --output /tmp/cpp.json
# Compare diagnostics emitted by puzzlescript_cpp (stderr?) against JS errorStrings.
```

For now, `compile` should print diagnostics to stderr in JS-compat-formatted form. Update the subcommand to do so.
...
```bash
git add native/src/compiler/parser_preamble.cpp native/src/compiler/diagnostic.cpp \
        native/src/cli/main.cpp docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: preamble diagnostics (unrecognised/title-too-long)"
```
...
```bash
git add native/src/compiler/parser.cpp
git commit -m "compiler: section header detection + dispatch routing"
```
...
**Files:**
- Modify: `native/src/compiler/parser.cpp`
- Modify: `native/src/compiler/diagnostic.cpp`
...
```bash
git add native/src/compiler/parser.cpp native/src/compiler/diagnostic.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: section header diagnostics"
```
...
**Files:**
- Create: `native/src/compiler/parser_objects.cpp`
...
```bash
git add native/src/compiler/parser_objects.cpp native/src/compiler/parser.cpp native/CMakeLists.txt
git commit -m "compiler: OBJECTS name-line parsing"
```
...
**Files:**
- Modify: `native/src/compiler/parser_objects.cpp`
- Modify: `native/src/compiler/language_constants.cpp` (populate color table — see Task 4 step 2 TODO)
...
```bash
git add native/src/compiler/parser_objects.cpp native/src/compiler/language_constants.cpp
git commit -m "compiler: OBJECTS color-line parsing + color name table"
```
...
```bash
git add native/src/compiler/parser_objects.cpp native/src/compiler/diagnostic.cpp
git commit -m "compiler: OBJECTS sprite-matrix parsing + 5x5 diagnostics"
```
...
```bash
git add native/src/compiler/parser_objects.cpp docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: OBJECTS section — handle all corpus edge cases"
```
...
**Files:**
- Create: `native/src/compiler/parser_legend.cpp`
...
```bash
git add native/src/compiler/parser_legend.cpp native/src/compiler/parser.cpp native/CMakeLists.txt
git commit -m "compiler: LEGEND synonyms"
```
...
**Files:**
- Modify: `native/src/compiler/parser_legend.cpp`
...
```bash
git add native/src/compiler/parser_legend.cpp
git commit -m "compiler: LEGEND aggregates + properties"
```
...
```bash
git add native/src/compiler/parser_legend.cpp native/src/compiler/diagnostic.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: LEGEND diagnostics complete"
```
...
**Files:**
- Create: `native/src/compiler/parser_sounds.cpp`
...
```bash
git add native/src/compiler/parser_sounds.cpp native/CMakeLists.txt
git commit -m "compiler: SOUNDS basic events + verbs"
```
...
**Files:**
- Modify: `native/src/compiler/parser_sounds.cpp`
- Modify: `native/src/compiler/diagnostic.cpp`
...
```bash
git add native/src/compiler/parser_sounds.cpp native/src/compiler/diagnostic.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: SOUNDS diagnostics + edge cases"
```
...
**Files:**
- Create: `native/src/compiler/parser_collisionlayers.cpp`
...
```bash
git add native/src/compiler/parser_collisionlayers.cpp native/CMakeLists.txt
git commit -m "compiler: COLLISIONLAYERS basic parsing"
```
...
**Files:**
- Modify: `native/src/compiler/parser_collisionlayers.cpp`
- Modify: `native/src/compiler/diagnostic.cpp`
...
```bash
git add native/src/compiler/parser_collisionlayers.cpp native/src/compiler/diagnostic.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: COLLISIONLAYERS diagnostics + corpus progress"
```
...
**Files:**
- Create: `native/src/compiler/parser_rules.cpp`
...
```bash
git add native/src/compiler/parser_rules.cpp native/CMakeLists.txt
git commit -m "compiler: RULES — basic lexing (brackets/pipes/arrow)"
```
...
**Files:**
- Modify: `native/src/compiler/parser_rules.cpp`
...
```bash
git commit -m "compiler: RULES — cell content tokens"
```
...
```bash
git commit -m "compiler: RULES — direction prefixes"
```
...
```bash
git add native/src/compiler/parser_rules.cpp
git commit -m "compiler: RULES — late / rigid modifiers"
```
...
```bash
git add native/src/compiler/parser_rules.cpp
git commit -m "compiler: RULES — RHS commands (sfx/win/message/cancel/again)"
```
...
```bash
git add native/src/compiler/parser_rules.cpp
git commit -m "compiler: RULES — loops + ellipsis"
```
...
```bash
git add native/src/compiler/parser_rules.cpp native/src/compiler/diagnostic.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: RULES — diagnostics + full pass"
```
...
**Files:**
- Create: `native/src/compiler/parser_winconditions.cpp`
...
```bash
git commit -m "compiler: WINCONDITIONS basic"
```
...
**Files:**
- Modify: `native/src/compiler/parser_winconditions.cpp`
- Modify: `native/src/compiler/diagnostic.cpp`
...
```bash
git add native/src/compiler/parser_winconditions.cpp native/src/compiler/diagnostic.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: WINCONDITIONS diagnostics + corpus progress"
```
...
**Files:**
- Create: `native/src/compiler/parser_levels.cpp`
...
```bash
git commit -m "compiler: LEVELS — grid rows"
```
...
**Files:**
- Modify: `native/src/compiler/parser_levels.cpp`
...
```bash
git add native/src/compiler/parser_levels.cpp native/src/compiler/diagnostic.cpp
git commit -m "compiler: LEVELS — message screens + MessageMissingSpace warning"
```
...
git add native/src/compiler/parser_levels.cpp native/src/compiler/diagnostic.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: LEVELS — diagnostics + corpus progress"
```
...
```bash
git commit -m "compiler: full corpus ParserState parity (730/730 fixtures)"
```
...
- Modify: `native/src/cli/main.cpp` — ensure `compile` emits diagnostics to stdout (one per line, `format_for_js_compat` output) when `--emit-diagnostics` is passed.
...
```bash
git add scripts/diff_diagnostics_against_js.sh native/src/cli/main.cpp \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md
git commit -m "compiler: diagnostics parity gate — corpus clean"
```

---

## Task 44b: Implement the C API (`ps_compiler_parse` and accessors)

**Files:**
- Create: `native/src/compiler/c_api_compiler.cpp`
- Modify: `native/CMakeLists.txt` (add `c_api_compiler.cpp` to `PUZZLESCRIPT_COMPILER_SOURCES`)
- Reference: `native/include/puzzlescript/compiler.h` (from Task 2), existing `native/src/c_api.cpp` for the opaque-handle pattern.

The C header has existed since Task 2 as a forward-declared surface. Until now `puzzlescript_cpp` has called `puzzlescript::compiler::parse()` directly (in C++). Now we wire the C API so future consumers (LSP, etc.) can link against `libpuzzlescript_compiler.a` without including any C++ headers.

- [ ] **Step 1: Define the opaque result struct**

```cpp
// native/src/compiler/c_api_compiler.cpp
#include "puzzlescript/compiler.h"
#include "parser.hpp"
#include "diagnostic.hpp"
...
struct ps_compiler_result {
    puzzlescript::compiler::ParserState state;
...
extern "C" ps_compiler_result* ps_compiler_parse(const char* source, size_t source_len) {
    auto* r = new ps_compiler_result;
    std::string s(source, source_len);
    auto out = puzzlescript::compiler::parse(s);
    ...
    for (const auto& d : r->diagnostics) {
        r->diagnostic_texts.push_back(puzzlescript::compiler::format_for_js_compat(d));
        ps_diagnostic c{};
...
extern "C" size_t ps_compiler_result_diagnostic_count(const ps_compiler_result* r) {
    return r ? r->diagnostic_c.size() : 0;
}

extern "C" const ps_diagnostic* ps_compiler_result_diagnostic(
    const ps_compiler_result* r, size_t index)
{
    if (!r || index >= r->diagnostic_c.size()) return nullptr;
    return &r->diagnostic_c[index];
}

extern "C" size_t ps_compiler_result_parser_state_json(
    const ps_compiler_result* r, char* buf, size_t cap)
{
#ifdef PS_ENABLE_DEV_SERIALIZERS
    if (!r) return 0;
    std::string s = puzzlescript::compiler::serialize_parser_state(r->state);
    if (buf && cap > 0) {
...
extern "C" void ps_compiler_result_free(ps_compiler_result* r) { delete r; }
```
...
```cmake
set(PUZZLESCRIPT_COMPILER_SOURCES
  src/compiler/c_api_compiler.cpp
  src/compiler/diagnostic.cpp
  src/compiler/language_constants.cpp
  src/compiler/parser.cpp
  src/compiler/parser_collisionlayers.cpp
  src/compiler/parser_legend.cpp
  src/compiler/parser_levels.cpp
  src/compiler/parser_objects.cpp
  src/compiler/parser_preamble.cpp
  src/compiler/parser_rules.cpp
  src/compiler/parser_sounds.cpp
  src/compiler/parser_winconditions.cpp
)
```
...
Create `native/tests/compiler/test_c_api_smoke.c`:
...
int main(void) {
    ...
    ps_compiler_result* r = ps_compiler_parse(src, strlen(src));
    assert(r != NULL);
    /* Diagnostic count may be nonzero (e.g. missing Player); that's fine. */
    size_t n = ps_compiler_result_diagnostic_count(r);
    for (size_t i = 0; i < n; ++i) {
        const ps_diagnostic* d = ps_compiler_result_diagnostic(r, i);
        printf("[%d] line=%d: %s\n", (int)d->severity, d->line, d->message);
    }
    ps_compiler_result_free(r);
    printf("OK\n");
    return 0;
}
```
...
```cmake
add_executable(test_c_api_smoke tests/compiler/test_c_api_smoke.c)
set_target_properties(test_c_api_smoke PROPERTIES LINKER_LANGUAGE CXX)
target_link_libraries(test_c_api_smoke PRIVATE puzzlescript_compiler)
add_test(NAME compiler_c_api_smoke COMMAND test_c_api_smoke)
```

- [ ] **Step 4: Build and run**

```
cmake --build build --target test_c_api_smoke && ctest --test-dir build -R compiler_c_api_smoke --output-on-failure
```
...
```bash
git add native/src/compiler/c_api_compiler.cpp native/tests/compiler/test_c_api_smoke.c native/CMakeLists.txt
git commit -m "compiler: implement C API (ps_compiler_parse + accessors)"
```
...
- Modify: `scripts/perf_check.sh` (add sanity check that `compile` completes on a median fixture in <100ms)
...
git commit -m "compiler: CI gate for parser corpus parity + sanity perf check"
```
...
**Files:**
- Delete: `native/src/compiler/placeholder.cpp`
- Modify: `native/CMakeLists.txt` — remove placeholder source
- Modify: `docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md` — mark complete
- Create: `docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-results.md` — end-of-phase summary
...
```bash
git add native/src/compiler/placeholder.cpp native/CMakeLists.txt \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-progress.md \
        docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-results.md
git commit -m "compiler: Phase 1 complete — parser + harness live, corpus green"
```
...
- [ ] All `ctest -R compiler_` tests pass.
...
- [ ] `native/src/compiler/` matches the File Structure section of this plan (directory layout is conventional).
- [ ] `docs/superpowers/plans/2026-04-22-cpp-compiler-phase1-results.md` documents final LOC, effort, and any P2-relevant findings.
...
- `puzzlescript_cpp run <source.txt>` — stays on the JS-generated `ir.json` path until P4.
