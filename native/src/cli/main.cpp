...
using puzzlescript::compiler::safeCppIdentifier;
...
std::string generateCompiledRulesCoverageJson(
    ...
    for (size_t index = 0; index < sources.size(); ++index) {
        ...
        appendCompactTurnSourceJsonFields(
            out,
            sources[index].game
                ? compactTurnSupportForGame(*sources[index].game, compactOptions)
                : CompactTurnSupport{}
        );
        out << "}";
...
int compileRulesCommand(const std::string& sourcePath, int argc, char** argv) {
    ...
    for (int index = 0; index < argc; ++index) {
        ...
        if (arg == "--emit-cpp") {
            ...
            if (mode == "interpreter") {
                compactOptions.interpreterMode = true;
            } else if (mode == "compiler") {
                compactOptions.interpreterMode = false;
            } else {
                throw std::runtime_error("--compact-turn-mode requires interpreter or compiler");
            }
        } else if (arg.rfind("--compact-turn-mode=", 0) == 0) {
            const std::string mode = arg.substr(std::string("--compact-turn-mode=").size());
            if (mode == "interpreter") {
                compactOptions.interpreterMode = true;
            } else if (mode == "compiler") {
                compactOptions.interpreterMode = false;
    ...
    if (coverageJson.has_value()) {
        writeFileIfChanged(
            *coverageJson,
            generateCompiledRulesCoverageJson(sources, options, compactOptions, coverage)
        );
    }
    if (statsOnly) {
    ...
    if (emitCpp.has_value()) {
        const std::string generated = generateCompiledRulesCpp(sources, symbol, options, compactOptions, true, {}, {}, {}, compactTurnOnly);
    ...
    std::cerr << "compiled-rules: sources=" << sources.size()
              ...
              << " compact_turn_mode=" << (compactOptions.interpreterMode ? "interpreter" : "compiler")
              << " output=" << outputPath
              << " wrote=" << wroteCount
              << "\n";
    if (maxCompiledRulesPerSource.has_value() && skippedCompiledRules > 0) {
