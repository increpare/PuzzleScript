...
void emitCollectRowMatches(
    ...
    out << "    int32_t xmin_" << rowIndex << " = 0;\n"
        << "    int32_t xmax_" << rowIndex << " = session.scratch.liveLevel.width;\n"
        << "    int32_t ymin_" << rowIndex << " = 0;\n"
        << "    int32_t ymax_" << rowIndex << " = session.scratch.liveLevel.height;\n";
    switch (rule.direction) {
        case 1:
    ...
    if (!anchorCandidates.empty()) {
        ...
        out << "    }\n"
            ...
            << "                        const int32_t anchorX = anchorTile / session.scratch.liveLevel.height;\n"
            << "                        const int32_t anchorY = anchorTile % session.scratch.liveLevel.height;\n"
        ...
        out << "                        const int32_t startIndex = startX * session.scratch.liveLevel.height + startY;\n"
        ...
        if (horizontal) {
            out << "            if (" << matchesName << ".size() > 1) {\n"
                ...
                << "                    const int32_t lhsX = lhs / session.scratch.liveLevel.height;\n"
                << "                    const int32_t lhsY = lhs % session.scratch.liveLevel.height;\n"
                << "                    const int32_t rhsX = rhs / session.scratch.liveLevel.height;\n"
                << "                    const int32_t rhsY = rhs % session.scratch.liveLevel.height;\n"
