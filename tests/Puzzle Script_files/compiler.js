...
function collapseRules(state) {
	for (var i = 0; i < state.rules.length; i++)
	{
		...
		var newrule = [0,[],[],oldrule.lineNumber,oldrule.late/*ellipses,group number,rigid, rule number in group added in the next phase*/];
		var ellipses = [];
...
function arrangeRulesByGroupNumber(state) {
	...
	for (var i=0;i<state.rules.length;i++) {
		...
		rule.push(targetArray[groupNumber].length);
...
function generateRigidGroupList(state) {
	var group_number_to_group_index = new Array();
	var absolute_to_rigid_rule_index = new Array(state.rules.length);
	var rigid_to_absolute_rule_index = new Array(state.rules.length);

	for (var groupIndex=0;groupIndex<state.rules.length;groupIndex++) {
		absolute_to_rigid_rule_index[groupIndex]=[];
		rigid_to_absolute_rule_index[groupIndex]=[];
	}

	var rigidGroupCount=0;
	for (var absoluteGroupIndex=0;absoluteGroupIndex<state.rules.length;absoluteGroupIndex++) {
		var rigidFound=0;
		var ruleset=state.rules[absoluteGroupIndex];
		group_number_to_group_index[ruleset[0][6]]=absoluteGroupIndex;
		var rigidRuleCount=0;			
		for (var absoluteRuleIndex=0;absoluteRuleIndex<ruleset.length;absoluteRuleIndex++) {
			var rule=ruleset[absoluteRuleIndex];
			if (rule[7]) {
				absolute_to_rigid_rule_index[absoluteGroupIndex][absoluteRuleIndex]=[rigidGroupCount,rigidRuleCount];
				rigid_to_absolute_rule_index[rigidGroupCount][rigidRuleCount]=[absoluteGroupIndex,absoluteRuleIndex];
				rigidRuleCount++;
				if (rigidRuleCount>30) {
					logError("There can't be more than 30 rigid rules in a single execution group",rules[0][0][3]);
				}
			}
		}
		if (rigidFound) {
	...
	if (rigidGroupCount>30) {
		logError("There can't be more than 30 rigid groups (rule groups containing rigid members).",rules[0][0][3]);
	}

	state.group_number_to_group_index = group_number_to_group_index;
	state.absolute_to_rigid_rule_index = absolute_to_rigid_rule_index;
	state.rigid_to_absolute_rule_index = rigid_to_absolute_rule_index;
}
