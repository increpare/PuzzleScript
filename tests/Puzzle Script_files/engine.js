...
function loadLevelFromState(state,levelindex) {	
    ...
    if (leveldat.message===undefined) {
	    ...
	    level = {
	        ...
	        rigidRuleIndexMask: leveldat.dat.concat([])//group index
	    };
...
var bannedRule = [];//uses absolute skeleton keys

function commitPreservationState(ruleGroupIndex,ruleIndex) {
	var propagationState = {
		...
		rigidRuleIndexMask:level.rigidRuleIndexMask.concat([]),//[[mask,groupNumber]
        rigidMovementAppliedMask:level.rigidMovementAppliedMask.concat([]),
		rigidBackups:rigidBackups.concat([]),
		bannedRule:bannedRule.concat([])
	};
	var absoluteKey = ruleGroupIndex+ruleIndex*state.rules.length;
        ...
        for (var cellRowIndex=0;cellRowIndex<rule[1].length;cellRowIndex++) {
            ...
            for (var cellIndex=0;cellIndex<preRow.length;cellIndex+=5) {
				...
				if (rule[7]) {
            		...
            		var ruleIndex = rule[8];
            		var groupIndex = state.group_number_to_group_index[groupNumber];
            		var rigidGroupPos = state.absolute_to_rigid_rule_index[groupIndex][ruleIndex];
            		var rigidGroupIndex  = rigidGroupPos[0];
            		var rigidRuleIndex = rigidGroupPos[1];
            		rigidGroupIndex++;//don't forget to -- it when decoding :O              	
            		rigidRuleIndex++;//don't forget to -- it when decoding :O              	
            		var rigidGroupIndexMask = 
            					(rigidGroupIndex) +
            					((rigidGroupIndex<< ( 1 * 5 ))) +
            					((rigidGroupIndex<< ( 2 * 5 ))) +
            					((rigidGroupIndex<< ( 3 * 5 ))) +
            		...
            		rigidGroupIndexMask = rigidGroupIndexMask & postCell_MovementsLayerMask;
            		rigidRuleIndexMask = rigidRuleIndexMask & postCell_MovementsLayerMask;

            		curRigidGroupIndexMask = level.rigidGroupIndexMask[currentIndex];
            		curRigidRuleIndexMask = level.rigidRuleIndexMask[currentIndex];
            		curRigidMovementAppliedMask = level.rigidMovementAppliedMask[currentIndex];

            		var oldrigidGroupIndexMask = curRigidGroupIndexMask;
            		var oldrigidRuleIndexMask = curRigidRuleIndexMask;
            		var oldRigidMovementAppliedMask = curRigidMovementAppliedMask;

            		//if another rigid rule has already put something here, don't make any changes
            		//if ((curRigidRuleIndexMask&postCell_MovementsLayerMask)===0) {
            		if (true) {
	            		curRigidGroupIndexMask = curRigidGroupIndexMask & (~postCell_MovementsLayerMask);
	            		curRigidRuleIndexMask = curRigidRuleIndexMask & (~postCell_MovementsLayerMask);
	            		curRigidMovementAppliedMask = curRigidMovementAppliedMask & (~postCell_MovementsLayerMask);

	            		curRigidGroupIndexMask = curRigidGroupIndexMask | rigidGroupIndexMask;
	            		curRigidRuleIndexMask = curRigidRuleIndexMask | rigidRuleIndexMask;

	            		curRigidMovementAppliedMask = curRigidMovementAppliedMask | postCell_MovementsLayerMask;

	            		if (oldrigidGroupIndexMask!==curRigidGroupIndexMask ||
	            			oldRigidMovementAppliedMask !== curRigidMovementAppliedMask ||
	            			oldrigidRuleIndexMask !== curRigidRuleIndexMask) {
	            			rigidchange=true;
	            		}
            		} 
            	}
                ...
                if (oldCellMask!==curCellMask || oldMovementMask!=curMovementMask || rigidchange) {                	
                    ...
                    if (rigidchange) {
            			...
            			level.rigidRuleIndexMask[currentIndex] = curRigidRuleIndexMask;
    ...
    for (var ruleGroupIndex=startRuleGroupindex;ruleGroupIndex<state.rules.length;ruleGroupIndex++) {
        ...
        while(propagated) {
            ...
            for (var ruleIndex=0;ruleIndex<ruleGroup.length;ruleIndex++) {
                ...
                if (rule[7]) 
            	{
            		var absoluteKey =  ruleIndex*state.rules.length+ruleGroupIndex;

            		if (bannedRule[absoluteKey]) {
            			continue;
            		}

            		if (rigidBackups[absoluteKey]===undefined) {
            			rigidBackups[absoluteKey] = commitPreservationState(ruleGroupIndex,ruleIndex);
            		}
			    }
    		...
    		if (movementMask_restricted!==0) {
    			...
    			for (var j=0;j<6;j++) {
    				...
    				if (layerSection!==0) {
    					...
    					var rigidRuleIndexMask = level.rigidRuleIndexMask[i];
    					var rigidGroupIndex = parseInt("11111",2)&(rigidGroupIndexMask>>(5*j));
    					var rigidRuleIndex = parseInt("11111",2)&(rigidRuleIndexMask>>(5*j));
    					rigidGroupIndex--;//group indices start at zero, but are incremented for storing in the bitfield
    					rigidRuleIndex--;
    					var absolutePos = state.rigid_to_absolute_rule_index[rigidGroupIndex][rigidRuleIndex];
    					var groupIndex = absolutePos[0];
    					var ruleIndex = absolutePos[1];
    					var absoluteKey = ruleIndex*state.rules.length+groupIndex;
    					bannedRule[absoluteKey]=true;
    					backtrackTarget = rigidBackups[absoluteKey];
    					backtrackTarget.bannedRule[absoluteKey]=true;
    					break;
    				}
    			}
    		}
...
function processInput(dir) {
    ...
    if (dir<=4) {
        ...
        //after each iteration
        	...
        	rigidloop=false;
        	i++;
        	propagateMovements(startRuleGroupIndex,startRuleIndex);	
        	var rigidGroupUndoDat = resolveMovements();

        	if (rigidGroupUndoDat!=null) {
        		rigidloop=true;
        		restorePreservationState(rigidGroupUndoDat);
        		startRuleGroupIndex=rigidGroupUndoDat.ruleGroupIndex;
        		startRuleIndex=rigidGroupUndoDat.startRuleIndex+1;//
        	} else {
